...
if __name__ == "__main__":
    ...
    try:
        ...
        for table_name in ["fact_plays", "dim_songs", "dim_albums", "dim_artists", "dim_sources", "music_raw"]:
            client.command(T1Queries.DROP.do(f"temp_{table_name}"))
            client.command(T1Queries.CREATETEMP.do(table_name))
            ...
            client.command(T1Queries.INSERTTEMP.do(table_name, new_table))
            
            # Get new rows from raw for the producer buffer
            if table_name == "music_raw":
                new_rows = client.query(T1Queries.GETNEWROWS.do(table_name))
                
                # Rename temp_ to _tfd for later iteration ... not a CDC because we don't keep snapshots
                client.command(T1Queries.DROP.do(table_name))
                client.command(T1Queries.RENAMETEMP.do(table_name))
